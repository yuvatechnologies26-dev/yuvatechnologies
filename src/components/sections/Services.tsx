      ...
      <div className="container mx-auto container-px">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {services.map(([icon, name, desc]) => (
            <div
              key={name}
              className="group flex gap-4 p-5 rounded-2xl border border-border bg-card transition-all duration-300 hover:border-primary hover:-translate-y-1 hover:shadow-lift"
            >
              <div className="grid h-12 w-12 shrink-0 place-items-center rounded-xl bg-gradient-brand text-2xl shadow-soft">
                {icon}
              </div>
              <div>
